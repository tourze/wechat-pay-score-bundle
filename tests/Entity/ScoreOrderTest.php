...
class ScoreOrderTest extends TestCase
{
    ...
    public function testPostPaymentCollectionMethods(): void
    {
        ...
        $result = $this->scoreOrder->addScorePostPayment($postPayment);
        $this->assertSame($this->scoreOrder, $result); // 验证返回值是否为当前对象
        $this->assertCount(1, $this->scoreOrder->getPostPayments());
        $this->assertSame($this->scoreOrder, $postPayment->getScoreOrder()); // 验证关联关系是否已建立

        // 移除这个PostPayment
        $result = $this->scoreOrder->removePostPayment($postPayment);
        $this->assertSame($this->scoreOrder, $result); // 验证返回值是否为当前对象
        $this->assertCount(0, $this->scoreOrder->getPostPayments());
    ...
    public function testPostDiscountCollectionMethods(): void
    {
        ...
        $result = $this->scoreOrder->addPostDiscount($postDiscount);
        $this->assertSame($this->scoreOrder, $result); // 验证返回值是否为当前对象
        ...
        // 移除这个PostDiscount
        $result = $this->scoreOrder->removePostDiscount($postDiscount);
        $this->assertSame($this->scoreOrder, $result); // 验证返回值是否为当前对象
        $this->assertCount(0, $this->scoreOrder->getPostDiscounts());
